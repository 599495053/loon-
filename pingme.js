/*
 * PingMe_Task.js — Surge 专用版
 * 功能：多账号自动签到 + 看视频领金币
 * 依赖：本地持久化键 pingme_accounts_v1（由抓包脚本写入账号 Cookie/参数）
 *
 * [Script]
 * # 每天 8/12/18/22 点执行，可按需改 cron
 * PingMe任务 = type=cron,cronexp=0 8,12,18,22 * * *,timeout=300,script-path=PingMe_Task.js,script-update-interval=0
 * # 指定单个/多个邮箱（可选）：
 * # PingMe任务 = type=cron,cronexp=0 8 * * *,timeout=300,script-path=PingMe_Task.js,argument=you@mail.com
 *
 * [MITM]
...
// Surge 支持通过 cron argument / 面板传入指定邮箱
...
async function runAccount(acc, currentRunCount) {
    ...
    try {
        ...
        return { accountName, detail: msgs.join('\n'), initialBal, finalBal: "错误", checkInStr: "签×", videoCount, currentRunCount };
    }
}
...
(async () => {
    ...
    // 🚀 全自动适配：
    // 1. 若 cron/面板传入 argument，只跑对应邮箱
    // 2. 未传 argument 则跑本地全部账号
    if (targetEmail) {
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        let checkStatus = '×';
        if (res.checkInStr.includes('✓')) {
            checkStatus = '✓';
        } else if (res.checkInStr.includes('已签')) {
            checkStatus = '☑';
        }
    ...
    notify(scriptName, '✓ 运行完毕', notifyLines.join('\n'));
    console.log(`\n============= 全部任务结束 =============\n${consoleDetails.join('\n\n')}`);
